...
(async () => {
  ...
  try {
    for (const lang of ['es', 'en']) for (const width of [1440, 390, 320]) {
      ...
      await panel.locator('form').first().getByRole('textbox').fill('Technical help');
      ...
      conflict = true;
      await panel.locator('form').first().getByRole('textbox').fill('Preserve local edit');
      await button('Save draft', 'Guardar borrador').click();
      await panel.getByRole('alert').filter({ hasText: 'Draft changed' }).waitFor();
      assert.equal(await panel.locator('form').first().getByRole('textbox').inputValue(), 'Preserve local edit');
      await button('Reload draft', 'Recargar borrador').click();
      assert.equal(await panel.locator('form').first().getByRole('textbox').inputValue(), 'Preserve local edit', 'cancel reload keeps edits');
      ...
      reject = true;
      await panel.locator('form').first().getByRole('textbox').fill('Rejected draft');
      await button('Save draft', 'Guardar borrador').click();
      await panel.getByRole('alert').filter({ hasText: 'cycle' }).waitFor();
      assert.equal(await panel.locator('form').first().getByRole('textbox').inputValue(), 'Rejected draft');
      reject = false;
      ...
      await panel.locator('form').first().getByRole('textbox').fill('Newer local edit');
      const late = page.waitForResponse(r => r.url().endsWith('/handoffs'));
      release(); await late;
      await page.evaluate(() => new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve))));
      assert.equal(await panel.locator('form').first().getByRole('textbox').inputValue(), 'Newer local edit', 'late initial GET cannot replace newer edits');
