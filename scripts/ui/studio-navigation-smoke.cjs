...
(async () => {
  ...
  try {
    for (const lang of ['es', 'en']) for (const theme of ['light', 'dark']) {
      ...
      const toggle = page.locator('[data-immersive-studio] > header > button');
      const showPicker = async () => { if (!await picker.isVisible()) await toggle.click(); };
      const choose = async id => { await showPicker(); await picker.selectOption(id); };
      await choose('b'); await page.waitForURL('**/studio/b');
      await page.locator('button[data-studio-node="agent:b-agent"]').waitFor();
      assert.equal(await page.locator('button[data-studio-node="agent:a-agent"]').count(), 0);
      ...
      page.once('dialog', d => d.dismiss()); await choose('a'); assert(page.url().endsWith('/studio/b'));
      await toggle.click();
      ...
      await choose('a'); assert(page.url().endsWith('/studio/b'));
      ...
      await toggle.click();
      await page.locator('button[data-studio-node="agent:b-agent"]').click();
      await page.getByLabel(es ? 'Nombre' : 'Name', { exact: true }).fill('Unsaved');
      page.once('dialog', d => d.dismiss()); await choose('a'); assert(page.url().endsWith('/studio/b'));
      page.once('dialog', d => d.accept()); await choose('a'); await page.waitForURL('**/studio/a');
      await showPicker();
