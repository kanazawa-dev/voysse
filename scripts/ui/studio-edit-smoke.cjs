...
(async () => {
  ...
  try {
    for (const lang of ['en', 'es']) {
      ...
      assert.equal(await page.locator('[data-studio-node="widget:b"]').evaluate(n => n.style.left), '900px', 'saved position survives hiding');
      ...
      assert.equal(await page.locator('[data-studio-node="agent:b"]').getAttribute('aria-pressed'), 'true');
