...
(async () => {
  ...
  try {
    for (const lang of ['en', 'es']) {
      ...
      const graph = { layout: { revision: 1, positions: { 'widget:b': { x: 900, y: 900 } }, zoom: 100 }, client: { id: 'client-a', name: 'Alpha', is_active: true }, agents: [agent('a', 'Sales'), agent('b', 'Support')], channels: ['whatsapp', 'whatsapp-cloud', 'instagram', 'messenger'].map(kind => ({ kind, id: kind, agent_id: 'a', status: 'connected', is_enabled: true, updated_at: '2026-09-06T00:00:00Z' })) };
      ...
      await page.locator('button[data-studio-node="channel:whatsapp"]').click();
      ...
      confirm = true;
      await page.getByRole('button', { name: es ? 'Aplicar conexión' : 'Apply connection' }).click();
      await page.waitForFunction(() => document.querySelector('button[data-studio-node="channel:whatsapp"]').textContent.includes('Support'));
      assert.equal(graph.channels[0].is_enabled, true);
      // Rebinding a channel is a drag from its connector handle onto the target agent.
      const cloudHandle = page.locator('[data-connector-handle][data-studio-node="channel:whatsapp-cloud"]');
      await cloudHandle.hover(); await page.mouse.down(); await page.locator('button[data-studio-node="agent:b"]').hover(); await page.mouse.up();
      await page.waitForFunction(() => document.querySelector('button[data-studio-node="channel:whatsapp-cloud"]').textContent.includes('Support'));
      ...
      await page.locator('button[data-studio-node="agent:b"]').click();
      ...
      assert.equal(await page.locator('[data-studio-node^="widget:"]').count(), 0);
      assert.equal(await page.locator('svg:has([data-studio-edge])').getAttribute('width'), '620', 'hidden widgets do not stretch canvas');
      const toggle = page.getByLabel(es ? 'Activar chat web' : 'Enable web chat', { exact: true });
      await toggle.check();
      assert.equal(await page.locator('[data-studio-node="widget:b"]').count(), 0, 'unsaved toggle does not change graph');
      await page.getByRole('button', { name: es ? 'Revisar y guardar' : 'Review and save' }).click();
      await page.locator('[data-studio-node="widget:b"]').waitFor();
      assert.equal(await page.locator('[data-studio-widget-edge="b"]').count(), 1);
      assert.equal(await page.locator('[data-studio-node="widget:b"]').locator('xpath=..').evaluate(n => n.style.left), '900px', 'saved position survives hiding');
      await page.locator('[data-studio-node="widget:b"]').click();
      await toggle.uncheck();
      await page.getByRole('button', { name: es ? 'Revisar y guardar' : 'Review and save' }).click();
      await page.locator('[data-studio-node="widget:b"]').waitFor({ state: 'detached' });
      assert.equal(await page.locator('[data-studio-widget-edge="b"]').count(), 0);
      assert.equal(await page.locator('button[data-studio-node="agent:b"]').getAttribute('aria-pressed'), 'true');
      ...
      await page.getByRole('button', { name: es ? 'Revisar y guardar' : 'Review and save' }).click();
      await page.locator('button[data-studio-node="agent:new"]').waitFor();
