...
(async () => {
  ...
  try {
    ...
    await page.route("**/api/**", async (route) => {
      ...
      let data = [];
      if (path === "/api/portal/demo") data = { client_name: "Customer", portal_title: "Customer Inbox", agency_name: "Test Agency", agency_brand_color: "#20365b" };
      ...
      else if (path === "/api/portal/demo/agents")
        data = [{ id: "agent-1", name: "Assistant" }];
      else if (path === "/api/portal/demo/conversations") data = [conversation];
      else if (path === "/api/portal/demo/conversations/conversation-1/reply") {
        ...
        data = conversation;
      } else if (path === "/api/portal/demo/conversations/conversation-1")
        data = conversation;
      else if (path.endsWith("/read")) return route.fulfill({ status: 204 });
      await route.fulfill({ json: data });
    });
    await page.goto(base + "/portal/demo");
    await page.locator('input[name="content"]').waitFor();
    ...
    await page.screenshot({
      path: "/tmp/voysse-portal-delivery.png",
      fullPage: true,
    });
