...
from ..services.handoffs import classify_hop
...
async def simulate(client_id: uuid.UUID, payload: Simulation, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    wrapper = {"simulation_only": True, "revision": draft["revision"], "source_agent_id": str(agent.id)}
    if not candidates:
        return {**wrapper, "target_agent_id": None, "rule_index": None, "condition": None, "reason": "", "outcome": "no_rules"}
    ...
    result = await classify_hop(db, agency_id=agency_id, agent_id=agent_id, provider=provider, model=model,
        credentials=credentials, candidates=candidates, message=payload.message, language=payload.language)
