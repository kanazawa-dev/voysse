...
function computeShimmerColors(totalWidth: number, shimmerPos: number, pulse: number): ShimmerColor[] {
  ...
  for (let i = 0; i < totalWidth; i++) {
    ...
    const baseR = 14 + (i / totalWidth) * 8;
    const baseG = 34 + (i / totalWidth) * 12;
    const baseB = 90 + (i / totalWidth) * 20;

    const r = baseR + (120 - baseR) * shimmer * 0.6;
    const g = baseG + (170 - baseG) * shimmer * 0.6;
...
export function ShimmerLoader({
  ...
  const [iconColor, setIconColor] = useState<[number, number, number]>([120, 167, 255]);
  ...
  useEffect(() => {
    ...
    function draw(ts: number) {
      ...
      const ic = done ? 160 : Math.round(80 + pulse * 20);

      setIcon(currentIcon);
      setIconColor([ic, done ? 190 : Math.round(150 + pulse * 15), 255]);
      if (showTokens) {
  ...
  return (
    <div className="inline-flex items-center gap-2" style={{ fontSize }}>
      <span className="leading-none" style={{ color: `rgb(${ir},${ig},${ib})`, fontSize: fontSize * 1.15 }}>
        {icon}
      </span>
      ...
      {showTokens ? (
        <span className="text-[#78a7ff]/70 tabular-nums" style={{ fontSize: fontSize * 0.85 }}>
          {tokens}k
        </span>
      ) : null}
    </div>
