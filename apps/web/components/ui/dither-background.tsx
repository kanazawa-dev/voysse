...
// `wrapEffect`'s type only captures RetroEffectImpl's constructor params, not the
// colorNum/pixelSize instance setters it also applies props onto at runtime.
const RetroEffect = wrapEffect(RetroEffectImpl) as unknown as React.ComponentType<RetroEffectProps>;
...
function DitheredWaves({
  ...
  useEffect(() => {
    if (material.current) {
      material.current.uniforms = waveUniformsRef.current;
  ...
  return (
    <>
      <mesh ref={mesh} scale={[viewport.width, viewport.height, 1]}>
        ...
        <shaderMaterial ref={material} vertexShader={waveVertexShader} fragmentShader={waveFragmentShader} />
      </mesh>
...
export default function DitherBackground({
  ...
  return (
    <Canvas className={className ? `dither-container ${className}` : "dither-container"} camera={{ position: [0, 0, 6] }} dpr={1} gl={{ antialias: true, powerPreference: "low-power" }}>
      <DitheredWaves
        waveSpeed={waveSpeed}
