...
import { Bot, FileText, ImageIcon, LoaderCircle, MessageSquarePlus, Send, TriangleAlert, UserRound, Wrench } from "lucide-react";
...
export function ChatPlayground({ lockedAgentId }: { lockedAgentId?: string }) {
  ...
  return <div className={`grid min-h-[calc(100vh-11rem)] overflow-hidden rounded-xl border bg-card lg:grid-cols-[18rem_1fr] ${lockedAgentId ? "mt-1 h-[650px]" : ""}`}>
    <aside className="flex min-h-0 flex-col border-r">
      {!lockedAgentId && <div className="grid gap-3 border-b p-4"><div className="grid gap-1.5"><Label>{t("playground.selectors.client")}</Label><Select items={clients.map((client) => ({ label: client.name, value: client.id }))} value={clientId || null} onValueChange={(value) => { const next = value ?? ""; setClientId(next); const first = agents.find((a) => a.client_id === next); setAgentId(first?.id || ""); }}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent>{clients.map((client) => <SelectItem key={client.id} value={client.id}>{client.name}</SelectItem>)}</SelectContent></Select></div><div className="grid gap-1.5"><Label>{t("playground.selectors.agent")}</Label><Select items={availableAgents.map((agent) => ({ label: agent.name, value: agent.id }))} value={agentId || null} onValueChange={(value) => setAgentId(value ?? "")}><SelectTrigger className="w-full"><SelectValue placeholder={t("playground.selectors.agentPlaceholder")} /></SelectTrigger><SelectContent>{availableAgents.map((agent) => <SelectItem key={agent.id} value={agent.id}>{agent.name}</SelectItem>)}</SelectContent></Select></div></div>}
    ...
    <section className="flex min-h-0 flex-col">
      <header className="flex items-center justify-between border-b p-4">{selectedAgent ? <div className="flex items-center gap-3"><span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary"><BloubAvatar size={42} seed={agentId} mood={busy ? "thinking" : "idle"} animated={busy} /></span><div><strong>{selectedAgent.name}</strong><small><i className={`mr-1 inline-block size-1.5 rounded-full ${selectedAgent.model ? "bg-emerald-500" : "bg-muted-foreground"}`} />{selectedAgent.model ? t("playground.chat.modelConfigured") : t("playground.chat.noModelConfigured")}</small></div></div> : <div><strong>{t("playground.chat.fallbackTitle")}</strong><small>{t("playground.chat.fallbackSubtitle")}</small></div>}{conversation && <span className={`rounded-full px-2 py-1 text-xs font-medium ${conversation.mode === "human" ? "bg-emerald-500/10 text-emerald-700" : "bg-primary/10 text-primary"}`}>{conversation.mode === "human" ? t("playground.chat.modeHuman") : t("playground.chat.modeAi")}</span>}</header>
      <ScrollArea className="min-h-0 flex-1 bg-muted/20"><div className="space-y-4 p-4">
        {!agentId ? <EmptyState icon={<Bot />} title={t("playground.empty.title")} description={t("playground.empty.description")} /> : !conversation?.messages?.length ? <div className="flex flex-1 flex-col items-center justify-center p-8 text-center text-muted-foreground"><BloubAvatar size={96} seed={agentId} mood="listening" /><h3 className="font-heading">{t("playground.welcome.title", { name: selectedAgent?.name || "" })}</h3><p>{t("playground.welcome.description")}</p>{!selectedAgent?.model && <Alert type="info">{t("playground.welcome.noModelAlert")}</Alert>}</div> : conversation.messages.map((message) => <div className={`flex gap-3 ${message.role === "assistant" ? "" : "flex-row-reverse"}`} key={message.id}><span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary">{message.role === "assistant" ? <BloubAvatar size={34} seed={agentId} animated={false} /> : <UserRound size={17} />}</span><div className="max-w-[85%]"><small>{message.sender_name || (message.role === "assistant" ? selectedAgent?.name : t("playground.message.you"))}</small><div className="rounded-xl bg-background p-3 text-sm shadow-sm">{message.content}</div>{message.sources?.length > 0 && <div className="mt-2 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground"><span className="inline-flex items-center gap-1 font-medium"><FileText size={13} />{t("playground.message.sourcesUsed")}</span>{message.sources.map((source) => <Badge key={source.id} variant="outline" title={source.excerpt}><FileText />{source.filename}</Badge>)}</div>}{(message.tool_calls?.length ?? 0) > 0 && <div className="mt-2 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground"><span className="inline-flex items-center gap-1 font-medium"><Wrench size={13} />{t("tools.usedInReply")}</span>{message.tool_calls!.map((call, index) => <Badge key={index} variant={call.is_error ? "destructive" : "outline"} title={call.result_preview}><Wrench />{call.name}</Badge>)}</div>}{message.tool_calls?.some((call) => call.is_error) && <div className="mt-2 rounded-lg bg-destructive/10 p-2 text-xs text-destructive">{message.tool_calls!.filter((call) => call.is_error).map((call, index) => <small key={index}><TriangleAlert size={12} /> <strong>{call.name}</strong> {call.result_preview}</small>)}</div>}</div></div>)}
        {busy && <div className="flex items-center gap-3" role="status"><BloubAvatar size={48} seed={agentId} mood="thinking" /><span className="text-sm text-muted-foreground">{t("shell.loading")}</span></div>}
        <div ref={endRef} />
      </div></ScrollArea>
