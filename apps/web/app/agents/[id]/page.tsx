...
export default function AgentDetailPage() {
  ...
  return <div className="page agent-detail-page">
    ...
    {tab === "details" && <form className="settings-form" onSubmit={saveConfig}>
      <section className="settings-section"><div className="settings-copy"><h3>{t("agents.detail.generalHeading")}</h3><p>{t("agents.detail.generalCopy")}</p></div><div className="settings-fields"><div className="form-grid"><label>{t("agents.detail.clientLabel")}<select name="client_id" defaultValue={agent.client_id}>{clients.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select></label><div className="flex flex-col gap-1.5"><Label htmlFor="agent-detail-name">{t("agents.detail.nameLabel")}</Label><Input id="agent-detail-name" name="name" required defaultValue={agent.name} /></div></div><div className="flex flex-col gap-1.5"><Label htmlFor="agent-detail-description">{t("agents.detail.descriptionLabel")}</Label><Textarea id="agent-detail-description" name="description" rows={3} defaultValue={agent.description} /></div></div></section>
      <section className="settings-section"><div className="settings-copy"><h3>{t("agents.detail.behaviorHeading")}</h3><p>{t("agents.detail.behaviorCopy")}</p></div><div className="settings-fields"><div className="flex flex-col gap-1.5"><Label htmlFor="agent-detail-instructions">{t("agents.detail.instructionsLabel")}</Label><Textarea id="agent-detail-instructions" name="instructions" rows={8} defaultValue={agent.instructions} placeholder={t("agents.detail.instructionsPlaceholder")} /></div><div className="flex flex-col gap-1.5"><Label htmlFor="agent-detail-personality">{t("agents.detail.personalityLabel")}</Label><Textarea id="agent-detail-personality" name="personality" rows={4} defaultValue={agent.personality} placeholder={t("agents.detail.personalityPlaceholder")} /></div></div></section>
    ...
    <section className="panel"><div className="panel-head"><div><h3>{t("agents.detail.qaHeading")}</h3><p>{t("agents.detail.qaCopy")}</p></div></div>
      <form className="qa-form" onSubmit={addQA}><Input name="question" required placeholder={t("agents.detail.qaQuestionPlaceholder")} /><Textarea name="answer" rows={2} required placeholder={t("agents.detail.qaAnswerPlaceholder")} /><Button type="submit" variant="secondary" className="align-start" disabled={busy}><Plus size={15} /> {t("agents.detail.qaAdd")}</Button></form>
    ...
    {tab === "widget" && <form className="settings-form" onSubmit={saveWidget}>
      ...
      <section className="settings-section"><div className="settings-copy"><h3>{t("agents.detail.widgetEmbedHeading")}</h3><p>{t("agents.detail.widgetEmbedCopy")}</p></div><div className="settings-fields">
        {widgetEnabled ? <>
          ...
          <div className="embed-actions"><Button type="button" variant="secondary" onClick={() => { navigator.clipboard.writeText(widgetSnippet); toast.success(t("agents.detail.widgetCopied")); }}><Copy size={15} /> {t("agents.detail.widgetCopy2")}</Button><Button variant="ghost" render={<a href={`/widget/${agent.widget_public_id}`} target="_blank" rel="noreferrer" />}><ExternalLink size={15} /> {t("agents.detail.widgetPreview")}</Button></div>
