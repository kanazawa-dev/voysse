...
export default function AgentsPage() {
  ...
  return <div className="page"><PageHead eyebrow={t("agents.list.eyebrow")} title={t("agents.list.title")} description={t("agents.list.description")} action={<Button render={<Link href="/agents/new" />}><Plus size={18} /> {t("agents.list.newAgent")}</Button>} />
    <div className="toolbar filters"><div className="relative w-full max-w-[420px]"><Search size={18} className="pointer-events-none absolute top-1/2 left-2.5 -translate-y-1/2 text-muted-foreground" /><Input className="pl-8" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t("agents.list.searchPlaceholder")} /></div><label className="filter-select">{t("agents.list.clientLabel")}<select value={clientId} onChange={(e) => setClientId(e.target.value)}><option value="">{t("agents.list.allClients")}</option>{clients.map((client) => <option value={client.id} key={client.id}>{client.name}</option>)}</select></label></div>
    {visible.length ? <div className="table-shell"><table className="data-table"><thead><tr><th>{t("agents.list.thAgent")}</th><th>{t("agents.list.thClient")}</th><th>{t("agents.list.thModel")}</th><th>{t("agents.list.thStatus")}</th><th /></tr></thead><tbody>{visible.map((agent) => <tr key={agent.id}><td><Link href={`/agents/${agent.id}`} className="entity-cell"><span className="agent-avatar"><Bot size={18} /></span><span><strong>{agent.name}</strong><small>{agent.description || t("agents.list.noDescription")}</small></span></Link></td><td><Link href={`/clients/${agent.client_id}`} className="table-link">{agent.client.name}</Link></td><td>{agent.model ? <span className="entity-cell"><strong>{agent.model}</strong><small>{providerLabel(agent.provider)}</small></span> : <span className="muted">{t("agents.list.notConfigured")}</span>}</td><td><StatusBadge active={agent.is_active} /></td><td><Link href={`/agents/${agent.id}`} className="row-arrow"><ArrowRight size={17} /></Link></td></tr>)}</tbody></table></div> : <EmptyState icon={<Bot />} title={t("agents.list.emptyTitle")} description={clients.length ? t("agents.list.emptyWithClients") : t("agents.list.emptyNoClients")} action={<Button render={<Link href={clients.length ? "/agents/new" : "/clients/new"} />}>{t("agents.list.continue")}</Button>} />}
  </div>;
}
