...
export default function ClientsPage() {
  ...
  return <div className="page">
    ...
    <div className="toolbar"><div className="relative w-full max-w-[420px]"><Search size={18} className="pointer-events-none absolute top-1/2 left-2.5 -translate-y-1/2 text-muted-foreground" /><Input className="pl-8" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t("clients.list.searchPlaceholder")} /></div><span className="result-count">{t("clients.list.resultCount", { count: visible.length })}</span></div>
