...
export default function WhatsAppCloudChannelPage() {
  ...
  return <div className="flex w-full flex-col gap-6">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.5fr_1fr]"><main>
      <Card className="p-5"><div className="mb-4 flex items-start justify-between gap-3"><span><Bot size={19} /></span><div><h2 className="font-heading">{t("clients.whatsapp.assignedAgent")}</h2><p>{t("clients.whatsapp.assignedAgentCopy")}</p></div></div><div className="flex items-center gap-3"><div className="grid min-w-64 gap-1.5"><Label>{t("clients.whatsapp.agentToRespond")}</Label><Select items={client.agents.map((agent) => ({ value: agent.id, label: `${agent.name}${agent.is_active ? "" : t("clients.whatsapp.inactiveSuffix")}` }))} value={agentId || null} onValueChange={(value) => setAgentId(value ?? "")} disabled={busy}><SelectTrigger className="w-full"><SelectValue placeholder={t("clients.whatsapp.selectAgent")} /></SelectTrigger><SelectContent>{client.agents.map((agent) => <SelectItem key={agent.id} value={agent.id}>{agent.name}{agent.is_active ? "" : t("clients.whatsapp.inactiveSuffix")}</SelectItem>)}</SelectContent></Select></div></div>{!client.agents.length && <Alert>{t("clients.whatsapp.needsAgent")}</Alert>}</Card>
