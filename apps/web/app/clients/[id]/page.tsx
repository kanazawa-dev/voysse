...
export default function ClientDetailPage() {
  ...
  return <div className="flex w-full flex-col gap-6">
    ...
    <header className="flex flex-col gap-4 sm:flex-row sm:items-center"><div className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary">{client.name.slice(0, 2).toUpperCase()}</div><div><div className="flex flex-wrap items-center gap-2"><h1 className="font-heading">{client.name}</h1><StatusBadge active={client.is_active} /></div><p>{client.industry || t("clients.detail.industryUndefined")} · {client.agents.length === 1 ? t("clients.detail.agentOne", { count: client.agents.length }) : t("clients.detail.agentMany", { count: client.agents.length })}</p></div><div className="flex flex-wrap items-center gap-2 sm:ml-auto"><Button variant="outline" render={<Link href={`/studio/${client.id}`} target="_blank" rel="noopener noreferrer" />}>Voysse Studio</Button><Button render={<Link href={`/agents/new?client=${client.id}`} />}><Bot size={17} /> {t("clients.detail.newAgent")}</Button></div></header>
