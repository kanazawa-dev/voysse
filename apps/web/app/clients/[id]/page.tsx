...
export default function ClientDetailPage() {
  ...
  return <div className="page">
    ...
    <header className="entity-header"><div className="entity-avatar xl">{client.name.slice(0, 2).toUpperCase()}</div><div><div className="title-line"><h1>{client.name}</h1><StatusBadge active={client.is_active} /></div><p>{client.industry || t("clients.detail.industryUndefined")} · {client.agents.length === 1 ? t("clients.detail.agentOne", { count: client.agents.length }) : t("clients.detail.agentMany", { count: client.agents.length })}</p></div><div className="header-actions"><Button render={<Link href={`/agents/new?client=${client.id}`} />}><Bot size={17} /> {t("clients.detail.newAgent")}</Button></div></header>
    ...
    {tab === "details" && <form className="page-form" onSubmit={saveDetails}><section className="form-section"><div className="section-copy"><h2>{t("clients.detail.clientInfo")}</h2><p>{t("clients.detail.clientInfoCopy")}</p></div><div className="form-fields"><div className="form-grid"><div className="flex flex-col gap-1.5"><Label htmlFor="client-detail-name">{t("clients.detail.name")}</Label><Input id="client-detail-name" name="name" required defaultValue={client.name} /></div><div className="flex flex-col gap-1.5"><Label htmlFor="client-detail-industry">{t("clients.detail.industry")}</Label><Input id="client-detail-industry" name="industry" defaultValue={client.industry} /></div></div><div className="flex flex-col gap-1.5"><Label htmlFor="client-detail-description">{t("clients.detail.descriptionLabel")}</Label><Textarea id="client-detail-description" name="description" rows={3} defaultValue={client.description} /></div><div className="flex flex-col gap-1.5"><Label htmlFor="client-detail-context">{t("clients.detail.generalContext")}</Label><Textarea id="client-detail-context" name="general_context" rows={9} defaultValue={client.general_context} /><span className="field-help">{t("clients.detail.generalContextHelp")}</span></div><label className="switch-row"><span><strong>{t("clients.detail.activeClient")}</strong><small>{t("clients.detail.activeClientHint")}</small></span><input name="is_active" type="checkbox" defaultChecked={client.is_active} /></label></div></section><div className="form-footer split"><Button type="button" variant="destructive" onClick={remove}><Trash2 size={16} /> {t("clients.detail.deleteClient")}</Button><Button type="submit" disabled={busy}>{busy ? <LoaderCircle className="spin" size={17} /> : <Save size={17} />} {t("clients.detail.saveChanges")}</Button></div></form>}

    {tab === "agents" && (client.agents.length ? <div className="table-shell"><table className="data-table"><thead><tr><th>{t("clients.detail.colAgent")}</th><th>{t("clients.detail.colFunction")}</th><th>{t("clients.detail.colStatus")}</th><th /></tr></thead><tbody>{client.agents.map((agent) => <tr key={agent.id}><td><Link className="entity-cell" href={`/agents/${agent.id}`}><span className="agent-avatar"><Bot size={18} /></span><strong>{agent.name}</strong></Link></td><td>{agent.description || t("clients.detail.noDescription")}</td><td><StatusBadge active={agent.is_active} /></td><td><Link className="row-arrow" href={`/agents/${agent.id}`}><ArrowRight size={17} /></Link></td></tr>)}</tbody></table></div> : <EmptyState icon={<Bot />} title={t("clients.detail.agentsEmptyTitle")} description={t("clients.detail.agentsEmptyDescription")} action={<Button render={<Link href={`/agents/new?client=${client.id}`} />}>{t("clients.detail.createAgent")}</Button>} />)}

    {tab === "channels" && <section className="compact-channel-grid"><article className="channel-live"><span><MessageCircle size={20} /></span><div><strong>{t("channels.whatsappCloud.title")}</strong><small>{t("clients.detail.channelWhatsappAvailable", { name: client.name })}</small></div><Button variant="secondary" render={<Link href={`/clients/${client.id}/channels/whatsapp-cloud`} />}>{t("clients.detail.configure")}</Button></article><article className="channel-live"><span><QrCode size={20} /></span><div><strong>{t("channels.whatsapp.title")}</strong><small>{t("clients.detail.channelWhatsappAvailable", { name: client.name })}</small></div><Button variant="secondary" render={<Link href={`/clients/${client.id}/channels/whatsapp`} />}>{t("clients.detail.configure")}</Button></article>{["Instagram", "Facebook Messenger", "Webchat"].map((name) => <article key={name}><span><MessageCircle size={20} /></span><div><strong>{name}</strong><small>{t("clients.detail.comingSoon")}</small></div><button disabled>{t("clients.detail.connect")}</button></article>)}</section>}

    {tab === "inbox" && <ClientInbox clientId={client.id} />}

    {tab === "portal" && <><form className="page-form" onSubmit={savePortal}><section className="form-section"><div className="section-copy"><h2>{t("clients.detail.portalTitle")}</h2><p>{t("clients.detail.portalCopy")}</p></div><div className="form-fields"><div className="flex flex-col gap-1.5"><Label htmlFor="portal-title">{t("clients.detail.portalTitleLabel")}</Label><Input id="portal-title" name="portal_title" defaultValue={client.portal_title} placeholder={t("clients.detail.portalTitlePlaceholder", { name: client.name })} /></div><div className="flex flex-col gap-1.5"><Label htmlFor="portal-slug">{t("clients.detail.portalUrl")}</Label><div className="slug-input"><span>localhost:3000/portal/</span><Input id="portal-slug" name="portal_slug" defaultValue={client.portal_slug} /></div></div><div className="url-preview"><code>{portalUrl}</code><button type="button" onClick={() => navigator.clipboard.writeText(portalUrl)}><Copy size={15} /> {t("clients.detail.copy")}</button>{client.portal_enabled && <a href={portalUrl} target="_blank"><ExternalLink size={15} /> {t("clients.detail.open")}</a>}</div><div className="form-grid"><div className="flex flex-col gap-1.5"><Label htmlFor="portal-email">{t("clients.detail.portalEmail")}</Label><Input id="portal-email" name="portal_email" type="email" defaultValue={client.portal_email || ""} placeholder={t("clients.detail.portalEmailPlaceholder")} /></div><div className="flex flex-col gap-1.5"><Label htmlFor="portal-password">{t("clients.detail.portalPassword")}</Label><Input id="portal-password" name="portal_password" type="password" autoComplete="new-password" placeholder={client.portal_password_configured ? t("clients.detail.portalPasswordKeep") : t("clients.detail.portalPasswordMin")} /></div></div><label className="switch-row"><span><strong>{t("clients.detail.publishPortal")}</strong><small>{t("clients.detail.publishPortalHint")}</small></span><input name="portal_enabled" type="checkbox" defaultChecked={client.portal_enabled} /></label></div></section><div className="form-footer"><Button type="submit" disabled={busy}>{busy ? <LoaderCircle className="spin" size={17} /> : <Save size={17} />} {t("clients.detail.savePortal")}</Button></div></form><PortalDomain clientId={client.id} /></>}
  </div>;
}
...
function PortalDomain({ clientId }: { clientId: string }) {
  ...
  return <section className="form-section domain-section"><div className="section-copy"><h2>{t("clients.detail.domainTitle")}</h2><p>{t("clients.detail.domainCopy")}</p></div><div className="form-fields">
    <label>{t("clients.detail.domainLabel")}<div className="domain-input"><Globe2 size={16} /><Input value={input} onChange={(e) => setInput(e.target.value)} placeholder="chat.brand.com" /><Button type="button" variant="secondary" onClick={save} disabled={busy || !input.trim()}><Save size={15} /> {t("clients.detail.domainSave")}</Button></div></label>
    {domain?.domain && <>
      ...
      {!domain.verified && <div className="dns-instructions">
        ...
        <div className="domain-actions"><Button type="button" onClick={verify} disabled={busy}>{busy ? <LoaderCircle className="spin" size={15} /> : <ShieldCheck size={15} />} {t("clients.detail.domainVerify")}</Button></div>
      </div>}
      <div className="form-footer"><Button type="button" variant="destructive" onClick={remove} disabled={busy}><Trash2 size={15} /> {t("clients.detail.domainRemove")}</Button></div>
    </>}
  </div></section>;
}
