...
This pulls `ghcr.io/kanazawa-dev/openvoiss-{api,web,whatsapp}:latest`. Pin a release
with `OPENVOISS_VERSION=v1.2.3 make pull`, or point at your own registry with
`OPENVOISS_IMAGE_PREFIX`. The prebuilt `web` image calls the API through the
...
| `TXT` | `_openvoiss-challenge.chat.brand.com` | the token shown |
...
docker compose --env-file .env.docker exec -T db \
  sh -c 'pg_dump -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Fc' > backups/openvoiss.dump
```
...
docker compose --env-file .env.docker exec -T db \
  sh -c 'pg_restore -U "$POSTGRES_USER" -d "$POSTGRES_DB" --clean --if-exists --no-owner' < backups/openvoiss.dump
...
```bash
cd apps/api && ../../.venv/bin/pytest -q     # backend (needs the openvoiss_test DB)
